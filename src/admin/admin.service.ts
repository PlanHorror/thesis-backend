...
export class AdminService {
  ...
  async updateStudentAccountService(
    ...
    const toCompare = (x: unknown): string => {
    ...
    const same = (a: unknown, b: unknown) => toCompare(a) === toCompare(b);
  ...
  async getAdminBroadcastNotificationsService() {
    try {
      return await this.notificationService.findByAdminBroadcast();
    } catch (error) {
      this.logger.error(
        "Failed to retrieve admin broadcast notifications",
        error,
      );
      throw error;
    }
  ...
  async markAdminBroadcastAsReadService(id?: string) {
    try {
      if (id) {
        return await this.notificationService.markAdminBroadcastAsReadById(id);
      }
      return await this.notificationService.markAllAdminBroadcastAsRead();
    } catch (error) {
      this.logger.error("Failed to mark admin notifications as read", error);
      throw error;
    }
  }
